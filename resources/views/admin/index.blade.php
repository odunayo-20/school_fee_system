            ...
            <div class="row">
              <div class="col-8">
                <div class="numbers">
                  ...
                  <p class="mb-0">
                    <span class="text-sm text-success font-weight-bolder">{{ \App\Models\ExternalStudent::whereDate('created_at', today())->count() }}</span>
                    new today
                  </p>
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-8">
                <div class="numbers">
                  ...
                  <p class="mb-0">
                    <span class="text-sm text-success font-weight-bolder">{{ \App\Models\FeeStructure::where('status', 0)->count() }}</span>
                    active structures
                  </p>
                </div>
              </div>
            ...
            <div class="row">
              <div class="col-8">
                <div class="numbers">
                  ...
                  <p class="mb-0">
                    <span class="text-sm text-success font-weight-bolder">{{ \App\Models\FeePayment::where('payment_method', 'online')->whereDate('created_at', today())->count() }}</span>
                    online payments
                  </p>
                </div>
              </div>
        ...
        <div class="card z-index-2 h-100">
          <div class="pt-3 pb-0 bg-transparent card-header">
            ...
            <p class="mb-0 text-sm">
              ...
              @php
                $previousMonth = \App\Models\FeePayment::whereMonth('created_at', now()->subMonth()->month)->sum('amount_paid');
                $currentMonth = \App\Models\FeePayment::whereMonth('created_at', now()->month)->sum('amount_paid');
                $percentageChange = $previousMonth > 0 ? (($currentMonth - $previousMonth) / $previousMonth) * 100 : 100;
              @endphp
              <span class="font-weight-bold">{{ number_format($percentageChange, 1) }}% {{ $percentageChange >= 0 ? 'more' : 'less' }}</span> than last month
            </p>
          </div>
          <div class="p-3 card-body">
            <div class="chart">
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            @php
                $months = collect(range(5, 0))->map(function($month) {
                    ...
                    return [
                        'month' => $date->format('M'),
                        'total' => \App\Models\FeePayment::whereYear('created_at', $date->year)
            ...
            @php
                $paymentMethods = \App\Models\FeePayment::whereMonth('created_at', now()->month)
